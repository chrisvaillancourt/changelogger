# plan to convert releases.sh prototype to a proper python cli tool

## approach
- Turn the logic in releases.sh:4-33 into a small Python CLI (e.g., changelogger) built with [cyclopts](https://cyclopts.readthedocs.io/en/stable/) so it can be installed via `uv tool install` and reused across repositories.
...
- Create a Python package (e.g., changelogger/cli.py) with a cyclopts command that accepts --owner, --repo, --start-tag, --end-tag, and --out.
...
- Publish to an internal package index or share via a git submodule; teammates can run uv tool install changelogger-tool and execute changelogger --owner ....
